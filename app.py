...
def fetch_cve_details():
    ...
    try:
        ...
        if response.status_code == 200:
            ...
            assigner = data.get("cve", {}).get("CVE_data_meta", {}).get("ASSIGNER", "No data")
            problemtype_data = data.get("cve", {}).get("problemtype", {}).get("problemtype_data", [])
            if problemtype_data and "description" in problemtype_data[0]:
                description_list = problemtype_data[0]["description"]
                problem_type = description_list[0].get("value", "No data") if description_list else "No data"
            else:
                problem_type = "No data"

            description_data = data.get("cve", {}).get("description", {}).get("description_data", [])
            description = description_data[0].get("value", "No data") if description_data else "No data"

            references = [
                ref.get("url", "No data")
                for ref in data.get("cve", {}).get("references", {}).get("reference_data", [])
            ]

            cvss_base_score = (
                data.get("impact", {}).get("baseMetricV3", {}).get("cvssV3", {}).get("baseScore", "No data")
            )
            cvss_severity = (
                data.get("impact", {}).get("baseMetricV3", {}).get("cvssV3", {}).get("baseSeverity", "No data")
            )
...
def register():
    if request.method == "POST":
        ...
        if not is_valid_password(password):
            return render_template("register.html", error="Password must be at least 8 characters long and contain at least one uppercase letter, one lowercase letter, one number, and one special character.")

        hashed_password = bcrypt.generate_password_hash(password).decode('utf-8')
        otp = generate_otp()
        users_collection.insert_one({'username': username, 'password': hashed_password, 'email': email, 'verified': False, 'otp': otp})

        msg = Message('Email OTP Verification', sender='your-email@example.com', recipients=[email])
...
def verify_otp():
    if request.method == "POST":
        ...
        if not email or not otp:
            return render_template("verify_otp.html", error="All fields are required.", email=email)
...
def login():
    if request.method == "POST":
        ...
        if user and bcrypt.check_password_hash(user['password'], password):
            ...
            session_collection.insert_one({'username': username, 'session_id': session.sid})
...
def logout():
    ...
    # Remove the session data from MongoDB
    session_collection = db['sessions']
    session_collection.delete_one({'session_id': session.sid})
    # Store the user's IP address in the database
...
def ip_lookup():
    if request.method == "POST":
        ...
        dict = {
            ...
            'asn': details.asn if hasattr(details, 'asn') else {'asn': 'No Data', 'name': 'No Data', 'domain': 'No Data', 'route': 'No Data', 'type': 'No Data'},
            'company': details.company if hasattr(details, 'company') else {'name': 'No Data', 'domain': 'No Data', 'type': 'No Data'},
            'privacy': details.privacy if hasattr(details, 'privacy') else {'vpn': 'No Data', 'proxy': 'No Data', 'tor': 'No Data', 'relay': 'No Data', 'hosting': 'No Data', 'service': 'No Data'},
            'abuse': details.abuse if hasattr(details, 'abuse') else {'address': 'No Data', 'country': 'No Data', 'email': 'No Data', 'name': 'No Data', 'network': 'No Data', 'phone': 'No Data'},
            'domains': details.domains if hasattr(details, 'domains') else {'ip': 'No Data', 'total': 'No Data', 'domains': ['No Data']}
        }
...
@app.route('/phonenumber_parser', methods=['GET', 'POST'])
def phonenumber_parser():
    if request.method == 'POST':
        ...
        try:
            ...
            return render_template('phonenumber_parser.html', error="Invalid phone number")
    else:
        return render_template('phonenumber_parser.html')
...
@app.route('/mac_address_lookup', methods=['GET', 'POST'])
def mac_address_lookup():
    if request.method == 'POST':
        mac_address = request.form['mac_address']
        if mac_address:
            # Perform MAC address lookup using the provided API
            headers = {
            ...
            response = requests.get(f'https://api.macvendors.com/v1/lookup/{mac_address}', headers=headers)
            if response.status_code == 200:
                data = response.json()
...
@app.route('/generaltools/text_to_ascii', methods=['GET', 'POST'])
def text_to_ascii():
    if request.method == 'POST':
        ...
        if action == 'text_to_ascii':
            ...
            if text:
                ...
                return render_template('text_to_ascii.html', ascii_result=ascii_result, original_text=text)
            else:
            ...
            if ascii_text:
                try:
                    ...
                    return render_template('text_to_ascii.html', text_result=text_result, original_ascii=ascii_text)
                except ValueError:
                    flash('Error: Invalid ASCII input. Please enter valid ASCII codes separated by spaces.')
            else:
                flash('Error: Please enter some ASCII codes for conversion.')
    return render_template('text_to_ascii.html')

@app.route('/generaltools/text_to_unicode', methods=['GET', 'POST'])
def text_to_unicode():
    if request.method == 'POST':
        action = request.form.get('action')
        if action == 'text_to_unicode':
            ...
            if text:
                ...
                return render_template('text_to_unicode.html', unicode_result=unicode_result, original_text=text)
            else:
                flash('Error: Please enter some text for conversion.')
            ...
            if unicode_input:
                try:
                    # Split the input into individual Unicode code points
                    code_points = unicode_input.strip().split()
                    text_result = ''.join(chr(int(code_point[2:], 16)) for code_point in code_points)
                    return render_template('text_to_unicode.html', text_result=text_result, original_unicode=unicode_input)
                except ValueError:
                    flash('Error: Invalid Unicode input. Please use format like "U+0041 U+0042".')
            else:
                flash('Error: Please enter Unicode code points for conversion.')
    return render_template('text_to_unicode.html')

@app.route('/generaltools/qr_code_generator', methods=['GET', 'POST'])
